...
## v1.4.0 (2026.09.24)
固定ヘッダーに隠れる一覧見出し、検索の解除・並び替え、公開日より前の追記表示を修正しました。Luna interactive 1.4.0をテーマに同梱し、単独版を停止した後も既存のグラフを表示できます。新しくアイキャッチに設定したJPEG・PNGは全サイズをWebPへ置き換え、変換成功後に元ファイルを削除します。
