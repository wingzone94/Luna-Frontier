...
<main id="primary" class="site-main lf-home">
	...
	<section class="lf-home__news" aria-labelledby="lf-news-title">
		<div class="lf-home__section-head">
			<h2 id="lf-news-title"><span class="material-symbols-outlined" aria-hidden="true">campaign</span><?php esc_html_e( '速報', 'luna-frontier' ); ?></h2>
			<p class="lf-home__intro"><?php esc_html_e( '最新のニュースをお届け', 'luna-frontier' ); ?></p>
			<?php if ( $lf_news_category instanceof WP_Term ) : ?>
			<a class="lf-home__more" href="<?php echo esc_url( get_category_link( $lf_news_category ) ); ?>"><?php esc_html_e( 'ニュース一覧を見る', 'luna-frontier' ); ?><span aria-hidden="true">→</span></a>
			<?php endif; ?>
		</div>
		...
		<?php if ( ! $lf_news ) : ?><p class="lf-home__empty"><?php esc_html_e( 'ニュース記事がまだありません。', 'luna-frontier' ); ?></p><?php endif; ?>
	</section>

	<div class="lf-home__columns">
		<section id="latest" class="lf-home__latest" aria-labelledby="lf-latest-title">
			<div class="lf-home__section-head">
				<h2 id="lf-latest-title"><?php esc_html_e( '新着記事', 'luna-frontier' ); ?></h2>
				<nav class="lf-home__filters" aria-label="新着記事のカテゴリー">
					<a href="<?php echo esc_url( $lf_home_url . '#latest' ); ?>"<?php echo 0 === $lf_selected ? ' aria-current="page"' : ''; ?>><?php esc_html_e( 'すべて', 'luna-frontier' ); ?></a>
				...
				<a class="lf-home__more" href="<?php echo esc_url( $lf_more_url ); ?>"><?php esc_html_e( 'もっと見る', 'luna-frontier' ); ?><span aria-hidden="true">→</span></a>
			</div>
			<div class="lf-home__story-list">
				...
				<?php if ( ! $lf_latest ) : ?><p class="lf-home__empty"><?php esc_html_e( '記事がまだありません。', 'luna-frontier' ); ?></p><?php endif; ?>
			</div>
		</section>
		...
		<aside class="lf-home__aside" aria-label="記事を探す">
			<section class="lf-home__panel" aria-labelledby="lf-popular-title">
				<div class="lf-home__section-head">
					<h2 id="lf-popular-title"><span class="material-symbols-outlined" aria-hidden="true">local_fire_department</span><?php esc_html_e( '人気記事', 'luna-frontier' ); ?></h2>
					<span class="lf-home__period"><?php esc_html_e( '過去7日間', 'luna-frontier' ); ?></span>
				</div>
				...
				<?php if ( ! $lf_popular ) : ?><p class="lf-home__empty"><?php esc_html_e( '閲覧データが集まると、人気記事を表示します。', 'luna-frontier' ); ?></p><?php endif; ?>
			</section>
			<?php if ( $lf_categories ) : ?>
			<section class="lf-home__panel" aria-labelledby="lf-categories-title">
				<div class="lf-home__section-head"><h2 id="lf-categories-title"><?php esc_html_e( 'カテゴリー', 'luna-frontier' ); ?></h2></div>
				<form class="lf-home__category-form" action="<?php echo esc_url( home_url( '/' ) ); ?>" method="get">
					<label class="screen-reader-text" for="lf-home-category"><?php esc_html_e( 'カテゴリーを選択', 'luna-frontier' ); ?></label>
					<?php wp_dropdown_categories( array(
						'name' => 'cat', 'id' => 'lf-home-category', 'hierarchical' => true,
						'hide_empty' => true, 'orderby' => 'name', 'show_option_none' => __( 'カテゴリーを選択', 'luna-frontier' ),
			...
			<section class="lf-home__panel lf-home__calendar" aria-labelledby="lf-calendar-title">
				<div class="lf-home__section-head"><h2 id="lf-calendar-title"><?php esc_html_e( 'カレンダー', 'luna-frontier' ); ?></h2></div>
