...
<main id="primary" class="site-main lf-spotlight-archive" aria-labelledby="lf-spotlight-title">
	<nav class="lf-spotlight-archive__breadcrumbs" aria-label="<?php esc_attr_e( 'パンくずリスト', 'luna-frontier' ); ?>">
		<a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'ホーム', 'luna-frontier' ); ?></a>
	...
	<header class="lf-spotlight-archive__header">
		...
		<h1 id="lf-spotlight-title"><?php esc_html_e( 'スポットライトアーカイブ', 'luna-frontier' ); ?></h1>
		<p class="lf-spotlight-archive__intro"><?php esc_html_e( '気になるテーマを、もっと深く。', 'luna-frontier' ); ?></p>
	</header>
	...
	<?php if ( $lf_features ) : ?>
			<section class="lf-spotlight-archive__collection" aria-labelledby="lf-spotlight-collection-title">
				<div class="lf-spotlight-archive__section-header">
					<h2 id="lf-spotlight-collection-title"><?php esc_html_e( 'これまでの特集', 'luna-frontier' ); ?></h2>
					<p><?php esc_html_e( 'テーマから探す', 'luna-frontier' ); ?></p>
				</div>
			...
			<article class="lf-spotlight-archive__feature">
				<a class="lf-spotlight-archive__link" href="<?php echo esc_url( $lf_feature['url'] ); ?>" aria-labelledby="lf-spotlight-feature-<?php echo esc_attr( (string) $lf_index ); ?>">
					...
					<div class="lf-spotlight-archive__text">
						<p class="lf-spotlight-archive__kicker"><?php esc_html_e( '特集', 'luna-frontier' ); ?></p>
						...
						<div class="lf-spotlight-archive__meta">
							<span><?php
								...
								printf( esc_html__( '%s件の記事', 'luna-frontier' ), esc_html( number_format_i18n( (int) ( $lf_feature['count'] ?? 0 ) ) ) );
							?></span>
							<span class="lf-spotlight-archive__action"><?php esc_html_e( '特集を読む', 'luna-frontier' ); ?><span aria-hidden="true"> →</span></span>
						</div>
					</div>
				</a>
			</article>
	...
	<?php else : ?>
		<div class="lf-spotlight-archive__empty">
			<p><?php esc_html_e( '現在、掲載中の特集はありません。', 'luna-frontier' ); ?></p>
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'ホームへ戻る', 'luna-frontier' ); ?></a>
		</div>
	<?php endif; ?>
</main>
<?php get_footer(); ?>
